...
analyze -format verilog {PPA.v}
elaborate PPA; # top module name

# store the unmapped results
write -hierarchy -format ddc -output results/PPA.unmapped.ddc
...
check_design > reports/PPA.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/PPA.mapped.v
write -format ddc -hierarchy -output results/PPA.mapped.ddc
write_sdf results/PPA.mapped.sdf
write_sdc -nosplit results/PPA.mapped.sdc

# Generate reports
report_qor > reports/PPA.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit \
  > reports/PPA.mapped.timing.rpt
report_area -nosplit > reports/PPA.mapped.area.rpt
report_power -nosplit > reports/PPA.mapped.power.rpt
